...
export default {
  1: (value, label, field, { formValue, labelShowTag }) => {
    ...
    return (
      <div key={fieldKey} className="dynamic-form-view-item">
        <div className="left">
          {labelShowTag ? (
            <span className={getClassName(labelShowTag)}>{label}</span>
          ) : null}
        </div>
        ...
        <div className="right">
          <div className="item-title item-radio-title">
            <ul>
              {labelValue &&
                labelValue.map((data) => {
                  const className = 'checkbox-icon-wrap-active';
                  return (
                    <li key={data} className={'item-radio-label'}>
  ...
  14: (
    ...
    if (
      ...
      return customControlRender[fieldKey](values, label, field, formValue);
    }

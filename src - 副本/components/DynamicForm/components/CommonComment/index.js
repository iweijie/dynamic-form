...
class CommonComment extends Component {
              ...
              <ul className={styles['common-comment-modal-list']}>
                {list.map((item) => (
                  ...
                  >
                    ...
                    <div className={styles['item-text']}>{item.presetText}</div>
                    {item.userId !== 0 ? (
                      <img
                        src={iconBin}
                ...
                {!list.length && <div className="no-select">当前无选项</div>}
              </ul>
            </div>
          ...
          apis
            ...
            .then((data) => {
              if (data && data.code === 0) {
                ...
                const temp = this.state.list.filter((item) => item.id !== id);
  ...
  handleEditClick = (e) => {
    ...
    const editItem = this.state.list.find((item) => item.id === this.editingId);

...
const imgLoaded = (e) => {
  const firstChild = document.querySelector('._custom_placeholder').children[0];
...
const Upload = ({ options, handler, form, onChange }) => {
  ...
  const upload = useCallback(
    (event) => {
      ...
      const text =
        fileAcceptIndex === -1 ? fileAcceptList[fileAcceptIndex].name : '文件';
      const {
        filePath = 'filePath',
      ...
      if (fileAccept === 'image/*') {
        ...
        if (findNotImg) {
          return Toast.fail(`${findNotImg.name}为非图片，请重新选择`, 2);
        }
      }

      if (maxUploadLength > 0 && files.length + uploadLen > maxUploadLength) {
        return Toast.fail(`选择${text}数量超过${maxUploadLength}`, 2);
      }

      for (let i = 0; i < files.length; i++) {
        if (files[i].size > maxMegabytes * 1024 * 1024) {
          return Toast.fail(`${text}${files[i].name}超过${maxMegabytes}M`, 2);
        }
      }
      let promiseInstance;
      ...
      setTimeout(() => {
        ...
        return promiseInstance.then((formData) => {
          zTool
            ...
            .then((re) => {
              if (re.code !== 0)
                return Toast.fail(re.msg || re.message || '上传失败！', 2);
            ...
            .catch((err) => {
              console.log('err:', err);
              Toast.fail(err.msg || err.message || '上传失败！', 2);
            })
            .finally(() => {
              target.value = null;
            });
    ...
    <div key={fieldKey} className="dynamic-form-upload-wrap">
      <div className="item-title">
        ...
        <ul key={fieldKey} className={'item-upload-wrap'}>
          {Array.isArray(values) &&
            values.map((item) =>
              fileAccept === 'image/*' ? (
                ...
                >
                  <img onLoad={handleImgLoaded} src={item.filePath} alt="" />
                  <div
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                <li
                  key={item.id}
                  onClick={() => (!_disabled ? handleImgDel(item) : null)}
          ...
          <li id="mobile-upload-btn" className="upload-item upload-btn">
            <input
              onChange={(event) => (_disabled ? null : upload(event, values))}
              ...
              (item) =>
                fileAccept === 'image/*' ? (
                  <li
                    ...
                    onClick={() => view(item)}
                  >
                    <img onLoad={imgLoaded} src={item.filePath} alt="" />
                  </li>
                ) : (
                  <li
                    onClick={() => view(item)}

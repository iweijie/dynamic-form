...
const SelectPersonCom = ({ field, form, expressionValue, closePopUp }) => {
  ...
  const isMultiple = _config.multiple === undefined ? true : !!_config.multiple;
      ...
      <ul key={fieldKey} className={'person-wrap'}>
        {values.map((value, index) => (
          <li
            ...
            onClick={() => handleDeletePerson(fieldKey, value, form)}
            className="person-item person-item-del"
          >
            <div className="person-item-round">{value.userName}</div>
          </li>
        ))}
        <li
          key="_personAddBtn"
  ...
  return (
    <div>
      ...
      {Array.isArray(values) && values.length ? (
        <ul key={fieldKey} className={'person-wrap'}>
          {values.map((value, index) => (
            <li key={value.id || value.userAccount} className="person-item">
              <div className="person-item-round">{value.userName}</div>
            </li>
          ))}
        </ul>

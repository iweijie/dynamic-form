    ...
    const interceptors = useMemo(
      () =>
        mapValues(defaultInterceptors, (defaultInterceptor, name) =>
          inject(
            props[name] === undefined ? defaultInterceptor : props[name],
            pickListenerArg,
          ),
        ),
      [defaultInterceptors],
    ...
    const lifeCycles = useMemo(() => {
      ...
      ReactComponentFunctionNames.forEach((name) => {
        if (DeclarativeComponent[name] !== undefined) {
          lifeCycles[name] = inject(DeclarativeComponent[name], pickRenderArg);
        } else {
        ...
        return (...runtimeArgs) => {
          ...
          if (preventDefault === true) {
            return listener !== undefined
              ? handleListenerResult(listener(listenerArg, ...runtimeArgs))
              : undefined;
          }
          ...
          if (preventDefaultOnFly) return handleListenerResult(nextState);

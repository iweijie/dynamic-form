...
function DynamicForm(props) {
  ...
  useEffect(() => {
    ...
    try {
      const asyncList = [];
      payload._linkages = payload._linkages ? JSON.parse(payload.linkages) : [];
      ...
      if (payload.sectionList) {
        payload.sectionList = payload.sectionList.map((section) => {
          ...
          formFieldInfoList.forEach((item) => {
            item._config = item._config || JSON.parse(item.config) || {};
            const { fieldKey, _config = {} } = item;
            const { selectList, selectListFieldNames = {} } = _config;
            ...
            if (formValues && formValues[fieldKey] !== undefined) {
              ...
              if (fieldsType[fieldKey] === 3) {
                item._labelInitialValue = formValues[`${fieldKey}Label`];
              }
            }
            // 下拉列表时候  添加 label 值
            if (
              ...
              for (let i = 0; i < selectList.length; i++) {
                if (selectList[i][value] === item.initialValue) {
                  item._labelInitialValue = selectList[i][label];
                }
              }
            }
            ...
            if (item.labelShowTag === null || item.labelShowTag === undefined) {
              item.labelShowTag = 1;
            }
            const { selectionsType } = item._config;
            if (+selectionsType === 2 && !isEmptyObject(fieldsType)) {
              asyncList.push(
    ...
    sectionList.forEach((section) => {
      ...
      formFieldInfoList.forEach((formFieldInfo) => {
        const isHiddenRendering =
          formFieldInfo._config && formFieldInfo._config.hiddenRendering;

        if (hiddenList.includes(formFieldInfo.fieldKey) && !isHiddenRendering)
          return;
    ...
    (values, isFilterNo = true) => {
      ...
      return Object.keys(values)
        .map((key) => {
          ...
          // 是否过滤空值
          if (isFilterNo && (values[key] === undefined || values[key] === null))
            return undefined;
          ...
          if (fieldType === 4) {
            ...
            if (bool) {
              value = getMultiDateValue(values[key], format, false);
            } else {
                    ...
                    const value =
                      formFieldType[key].fieldType === 5
                        ? Array.isArray(data[key])
                          ? data[key].length >= 2
                            ? [getDate(data[key][0]), getDate(data[key][1])]
                            : []
                          : getDate(data[key])
                        : data[key];
  ...
  const handleModalItemSelect = useCallback(
    (index, type) => {
      const modalContext = state.modalContext ? { ...state.modalContext } : {};
      ...
      {sectionList.map((item, parentIndex) => {
        ...
        const showList = formFieldInfoList
          .map((field) => {
            ...
            // 返回项 为 强制渲染的  或者非隐藏的
            return isHiddenRendering || !isInclude ? field : undefined;
          })
          .filter(Boolean);
          ...
          <div key={id} className={'dynamic-form-list'}>
            ...
            <List key={id}>
              {showList.map((field, index) => {
                ...
                // 再判断是否为 联动不禁用
                // 最后是自身携带的 是否禁用
                field._disabled = disabledList.includes(fieldKey)
                  ? true
                  : usableList.includes(fieldKey)
                  ? false
                  : !!disabled;
                field._required = requiredList.includes(fieldKey)
                  ? true
                ...
                return (
                  <div
                    key={fieldKey}
                    className={
                      _isHiddenRendering ? 'dynamic-form-list-item-none' : ''
                    }
                  >
                    {renderFieldItem(
                      {
            ...
            modalContext.content.map((item, index) => {
              ...
              return mode === 'multiple' ? (
                <CheckboxItem
                  key={id}
                  checked={
                    modalContext.value && modalContext.value.includes(value)
                  }
                ...
                <RadioItem
                  ...
                  onClick={
                    () => {
                      ...
                      handleChange(value, key, form);
                      handleChange(label, `${key}Label`, form);
                      handleModalToggle();
                    }
...
export default createForm({
  onValuesChange: (props, changed = {}, formValue) => {
    ...
    const { observer, fieldsType, customOnChange, form, formFieldMap } = props;
    console.log(props);
    if (observer) {
    ...
    customOnChange &&
      keys.forEach((key) => {
        if (customOnChange[key] && typeof customOnChange[key] === 'function') {
          const field = formFieldMap[key];
          const isList = field.fieldType === 3;
          const selectList = (field._config && field._config.selectList) || [];

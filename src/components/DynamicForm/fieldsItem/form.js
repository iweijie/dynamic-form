...
export default {
  1: ({ options, handler, form }) => {
    ...
    return (
      <div
        className={`${frequentWordKey ? 'frequent-word-input-key' : ''} ${
      ...
      >
        ...
        {frequentWordKey ? (
          <div className={'input-btns'}>
            <Button className={'input-btn'} type="button" onClick={toggle}>
              常用语
            </Button>
            <CommonComment
      ...
      >
        ...
        {frequentWordKey ? (
          <div className={'textarea-btns'}>
            <Button className={'textarea-btn'} type="button" onClick={toggle}>
              常用语
            </Button>
            <CommonComment
              formFieldName={formField.name}
  ...
  3: ({ options, handler, form }) => {
    ...
    return (
      <Item
        ...
        onClick={() => (_disabled ? null : handler.handleSelect(options, form))}
      >
        <span className={getClassName(labelShowTag)}>
          {_required ? requireSpan : null}
          {labelShowTag ? label : null}
          ...
          <ul>
            {selectList &&
              selectList.map((data) => {
                ...
                const onClick = () => {
                  if (hasValue) {
                    const index = value.findIndex((val) => val === data.value);
              ...
              selectList.map((data) => {
                const className =
                  value && value === data.value
                    ...
                    : 'radio-icon-wrap';
                const checkedSvg =
                  value && value === data.value ? svg.checked : svg.circle;
                const onClick = () => {
                ...
                return (
                  <li
                    ...
                    className={'item-radio-label'}
                  >
                    <div className={className}>{checkedSvg}</div>
                    <div>{data.label}</div>
                  </li>
                );
              })}

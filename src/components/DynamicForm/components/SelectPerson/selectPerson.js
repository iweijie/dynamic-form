...
function getCurrentData(unfoldKeys, datasource) {
  ...
  while (sign) {
    const findIndex =
      ...
      currentData.children.findIndex((data) => data.id === unfoldKeys[index]);
    const findData = currentData.children && currentData.children[findIndex];
  ...
  return (
    <li onClick={() => handleSelected(item)} key={id}>
      ...
      <div className={'item-right item-right-select'}>
        <p>
          {name || userName} {deptName ? <span>({deptName})</span> : null}
        </p>
      </div>
    </li>
  );
...
function SelectPerson(props) {
  ...
  const getUserList = useCallback(
    (keys) => {
      ...
      apis
        .getDepartmentAndUserList({ id: keys })
        .then((response) => {
  ...
  const searchText = searchkeys && searchkeys === searchValue ? '返回' : '搜索';
  return (
    <div className={'select-person-wrap'}>
      ...
      {isSelectedPerson ? (
        <div className={'select-person-search-input-wrap'}>
          ...
          <Button className="select-person-search-btn" onClick={onSearch}>
            {searchText}
          </Button>
        </div>
      ) : null}
          ...
          {hasPrevious ? (
            <div className="previous-btn-wrap">
              <div className="previous-btn" onClick={previousHandle}>
                ...
                <span className="previous-btn-text">返回上一页</span>
              </div>
            </div>
          ) : null}

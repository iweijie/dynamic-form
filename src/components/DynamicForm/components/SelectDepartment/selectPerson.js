    ...
    <li key={id} className="dynamic-form-department-item">
      ...
      >
        <div className={'item-left-arrow-wrap-padding'}>
          <span className={`item-left-arrow-wrap ${isChecked ? 'bgcfff' : ''}`}>
            {' '}
          </span>
        </div>
      </div>
...
const getPidPath = (id) =>
  ...
  apis.fetchUpDeptTree({ id }).then((data) => {
    ...
    while (data && data.id) {
      pids.push(data.id);
      const next = data.children && data.children.length && data.children[0];
      data = next;
    }
    return pids;
  });
    ...
    setDetList(rootId)
      .then(async (data) => {
        ...
        for (let i = selectedItems.length - 1; i >= 0; i--) {
          pids = await setTreeDataAboutTargetItems(selectedItems[i], [data]);
        }
        if (pids && pids.length) {

...
export const popUpImage = (function () {
      ...
      store.stie.addEventListener('mousedown', (event) => {
        if (event.button === 0) {
          ...
          const move = (e) => {
            ...
            topValue =
              topValue < h[0] ? h[0] : topValue > h[1] ? h[1] : topValue;
            leftValue =
              leftValue < w[0] ? w[0] : leftValue > w[1] ? w[1] : leftValue;
  ...
  function popUp(src, flag = false) {
    ...
    if (flag) {
      store.sh = sh =
        window.innerHeight || document.documentElement.clientheight;
      store.sw = sw = window.innerWidth || document.documentElement.clientWidth;
      css(mask, {
        width: `${sw}px`,
        height: `${sh}px`,
      });
